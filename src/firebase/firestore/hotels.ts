...
import { collection, addDoc, deleteDoc, doc, Firestore, writeBatch, getDocs, query, where } from 'firebase/firestore';
...
export async function deleteHotel(db: Firestore, hotelId: string) {
    ...
    const favoritesQuery = query(collection(db, 'users'), where('favorites', 'array-contains', hotelId));
