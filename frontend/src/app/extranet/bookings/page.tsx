          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>{bookings.map(b => (
              <tr key={b.id} style={{ borderBottom: '1px solid #F1F5F9' }}>
                ...
                <td style={{ padding: '14px 20px', color: '#64748B', fontSize: '0.85rem' }}>{new Date(b.checkin).toLocaleDateString()} → {new Date(b.checkout).toLocaleDateString()}</td>
                <td style={{ padding: '14px 20px', fontWeight: 700, color: '#FF1F71' }}>₹{b.totalPrice?.toLocaleString()}</td>
