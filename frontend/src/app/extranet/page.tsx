...
export default function ExtranetDashboard() {
  ...
  const cards = [
    { label: 'Total Hotels', value: stats?.totalHotels || 0, icon: '🏨', color: '#FF1F71' },
    { label: 'Total Rooms', value: stats?.totalRooms || 0, icon: '🛏️', color: '#6366F1' },
    { label: 'Total Bookings', value: stats?.totalBookings || 0, icon: '📋', color: '#059669' },
    { label: 'Avg Rating', value: stats?.avgRating || 0, icon: '⭐', color: '#F59E0B' },
  ];

  return (
    <div>
      <h1 style={{ fontSize: '1.8rem', fontWeight: 900, marginBottom: '8px' }}>Partner Dashboard</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', marginBottom: '32px' }}>
        <div style={{ background: 'linear-gradient(135deg, #FF1F71, #FF7E5F)', borderRadius: '16px', padding: '28px', color: 'white' }}>
          ...
          <div style={{ fontSize: '2rem', fontWeight: 900 }}>₹{(stats?.totalRevenue || 0).toLocaleString()}</div>
        </div>
        <div style={{ background: 'linear-gradient(135deg, #6366F1, #8B5CF6)', borderRadius: '16px', padding: '28px', color: 'white' }}>
          ...
          <div style={{ fontSize: '2rem', fontWeight: 900 }}>₹{(stats?.commission || 0).toLocaleString()}</div>
        </div>
        <div style={{ background: 'linear-gradient(135deg, #059669, #10B981)', borderRadius: '16px', padding: '28px', color: 'white' }}>
          ...
          <div style={{ fontSize: '2rem', fontWeight: 900 }}>₹{(stats?.partnerEarnings || 0).toLocaleString()}</div>
        </div>
      </div>
            ...
            {stats.recentBookings.map((b: any) => (
              <div key={b.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: '#FFF8F9', borderRadius: '10px' }}>
                <div>
                  ...
                  <div style={{ color: '#64748B', fontSize: '0.8rem' }}>{new Date(b.checkin).toLocaleDateString()} → {new Date(b.checkout).toLocaleDateString()}</div>
                </div>
                <div style={{ fontWeight: 700, color: '#FF1F71' }}>₹{b.totalPrice?.toLocaleString()}</div>
              </div>
            ))}
          </div>
        ) : (
