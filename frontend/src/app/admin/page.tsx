...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    { label: 'Pending Approvals', value: stats?.pendingHotels || 0, icon: '⏳', color: '#F59E0B' },
    { label: 'Partners', value: stats?.partnersCount || 0, icon: '🏢', color: '#8B5CF6' },
    { label: 'Guests', value: stats?.guestsCount || 0, icon: '🎫', color: '#3B82F6' },
  ];

  return (
    <div>
      <h1 style={{ fontSize: '1.8rem', fontWeight: 900, marginBottom: '8px' }}>Super Admin Dashboard</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        <div style={{ background: 'linear-gradient(135deg, #FF1F71, #FF7E5F)', borderRadius: '16px', padding: '32px', color: 'white' }}>
          ...
          <div style={{ fontSize: '2.5rem', fontWeight: 900 }}>₹{(stats?.totalRevenue || 0).toLocaleString()}</div>
        </div>
        <div style={{ background: 'linear-gradient(135deg, #059669, #10B981)', borderRadius: '16px', padding: '32px', color: 'white' }}>
          ...
          <div style={{ fontSize: '2.5rem', fontWeight: 900 }}>₹{(stats?.platformCommission || 0).toLocaleString()}</div>
        </div>
      </div>
    </div>
  );
